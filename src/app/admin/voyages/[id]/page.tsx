                ...
                <div className="sea-card p-6">
                    ...
                    <form action="/api/admin/voyages/update" method="POST" className="space-y-3">
                        ...
                        <div className="grid grid-cols-2 gap-3">
                            ...
                            <div><label className="block text-xs text-amber-600 mb-1">Status</label><select name="lifecycle" defaultValue={voyage.lifecycle || "Draft"} className="w-full px-3 py-2 rounded-lg bg-abyssal border border-amber-900/30 text-white text-sm">{["Draft", "Published", "Deprecated"].map(s => <option key={s} value={s}>{s}</option>)}</select></div>
